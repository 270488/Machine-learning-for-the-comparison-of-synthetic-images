...
PATH_TO_DATASET = 'Shadow_Reflections/'
num_views=5
num_renders=20

feature=["Reflections", "Shadow"]
...
#"strumenti","televisione", "scarpe", "palla",
#"umanoide", "libro",  "caffe", "scatole", "martello", "crema", "forbici"]
labels_test=["vino", "suzanne","bottiglia"]
...
def read_sentinel_img_trio_cm(path, obj, nw, nr):
    ...
    cm=np.stack((cm_shadow, cm_texture), axis=2)
...
def read_sentinel_img_trio_seg(path, obj, nw, nr, ref_ren):
    ...
    seg=np.stack((seg_shadow, seg_texture), axis=2)
...
weights_CD = torch.tensor([true_rate["shadow_CD"],true_rate["texture_CD"]],
                           dtype=torch.float32).view(2, 1, 1).cuda()

weights_LCM = torch.tensor([true_rate["shadow_LCM"], true_rate["texture_LCM"]],
                           dtype=torch.float32).view(2, 1, 1).cuda()
