...
PATH_TO_DATASET = 'Shadow_Textures_Transparency/'
num_views=5
num_renders=20

feature=["Shadow", "Texture", "Transparency"]
...
#"geometrie","strumenti","televisione", "scarpe", "palla",
#"umanoide", "libro",  "caffe","martello", "ombrello", "scatole", "bottiglia"]
labels_test=["forbici", "crema", "vino"]
labels_val=[ "vino", "suzanne", "carte"]
...
def read_sentinel_img_trio_cm(path, obj, nw, nr):
    ...
    cm_shadow = io.imread(path+str(nw)+"_"+nr_str+"_"+obj+"_CM"+feature[0]+".png", as_gray=True)
    cm_shadow=(cm_shadow!=0).astype(np.float32)
    
    cm_texture = io.imread(path+str(nw)+"_"+nr_str+"_"+obj+"_CM"+feature[1]+".png", as_gray=True)
    cm_texture=(cm_texture!=0).astype(np.float32)
    
    cm_transp = io.imread(path+str(nw)+"_"+nr_str+"_"+obj+"_CM"+feature[2]+".png", as_gray=True)
    cm_transp=(cm_transp!=0).astype(np.float32)

    cm=np.stack((cm_shadow, cm_texture, cm_transp), axis=2)
...
def read_sentinel_img_trio_seg(path, obj, nw, nr, ref_ren):
    ...
    seg_shadow = io.imread(path+str(nw)+"_"+nr_str+"_"+obj+"_"+ref_ren+"_"+feature[0]+".png", as_gray=True)
    ...
    seg_texture = io.imread(path+str(nw)+"_"+nr_str+"_"+obj+"_"+ref_ren+"_"+feature[1]+".png", as_gray=True)
    ...
    seg_transp = io.imread(path+str(nw)+"_"+nr_str+"_"+obj+"_"+ref_ren+"_"+feature[2]+".png", as_gray=True)
    seg_transp=(seg_transp/seg_transp.max()).astype(np.float32) if seg_transp.max()!=0 else seg_transp
    seg_transp=(seg_transp!=0).astype(np.float32)

    
    seg=np.stack((seg_shadow, seg_texture, seg_transp), axis=2)
    ...
    def __init__(self, path, train = True, patch_side = 96, stride = None, use_all_bands = False, transform=None):
        ...
        for obj_img in tqdm(self.names):
            for nw  in range(num_views):
                ...
                true_pix_shadow_LCM += (seg_reference[0].sum()) 
                true_pix_texture_LCM += (seg_reference[1].sum()) 
                true_pix_transp_LCM += (seg_reference[2].sum()) 
                ...
                for nr in range(1, num_renders):
                    ...
                    s = [cm.shape[1], cm.shape[2]]
                    n_pix += np.prod(s)
                    true_pix_shadow_CM += cm[0].sum() #0 ombre, 1 trasparenze, 2 texture
                    true_pix_texture_CM += cm[1].sum() #0 ombre, 1 trasparenze, 2 texture
                    true_pix_transp_CM += cm[2].sum() #0 ombre, 1 trasparenze, 2 texture
        ...
        self.transp_rate_CM=(n_pix-true_pix_transp_CM)/true_pix_transp_CM if true_pix_transp_CM!=0 else 0
        ...
        self.transp_rate_LCM=(2*n_pix-true_pix_transp_LCM)/true_pix_transp_LCM if true_pix_transp_LCM!=0 else 0
        
    def get_trueRate(self):
        return {'shadow_CD':self.shadow_rate_CM, 
                "shadow_LCM": self.shadow_rate_LCM,
                "texture_CD": self.texture_rate_CM, "texture_LCM": self.texture_rate_LCM,
                "transparency_CD": self.transp_rate_CM, "transparency_LCM": self.transp_rate_LCM}
...
weights_CD = torch.tensor([true_rate["shadow_CD"],true_rate["texture_CD"], true_rate["transparency_CD"]],
                           dtype=torch.float32).view(3, 1, 1).cuda()

weights_LCM = torch.tensor([true_rate["shadow_LCM"], true_rate["texture_LCM"], true_rate["transparency_LCM"]],
                           dtype=torch.float32).view(3, 1, 1).cuda()

print(weights_CD)
print(weights_LCM)
print(weights_CD/weights_CD.sum())
print(weights_LCM/weights_LCM.sum())
criterion_CD = nn.BCEWithLogitsLoss(weight=weights_CD/weights_CD.sum())
criterion_LCM = nn.BCEWithLogitsLoss(weight=weights_LCM/weights_LCM.sum())
...
if LOAD_TRAINED==False:
    ...
    for epoch in range(N_EPOCHS):
        ...
        if((train_loss[epoch]+val_loss[epoch]<min_loss and epoch%2==0)):
            ...
            loss_str="ChangeStar_model_loss.pth.tar"
...
with torch.no_grad():
    ...
    for i in range(len(images_names_test)):
        ...
        out_ref = ((out_ref>THRESHOLD) * 255).astype(np.uint8)
        out_ren = ((out_ren>THRESHOLD) * 255).astype(np.uint8)
        out_map = ((out_map>THRESHOLD) * 255).astype(np.uint8)
        ...
        for j, f in enumerate(feature):
            plt.imsave(f'{net_name}-{images_names_test[i]}_{feature[j]}_CM.png', out_map[j,:,:], cmap='gray')
            plt.imsave(f'{net_name}-{images_names_test[i]}_{feature[j]}_Reference.png', out_ref[j,:,:], cmap='gray')
            plt.imsave(f'{net_name}-{images_names_test[i]}_{feature[j]}_Render.png', out_ren[j,:,:], cmap='gray')
